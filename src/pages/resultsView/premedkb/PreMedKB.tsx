...
@observer
export default class PreMedKBViewer extends React.Component<
    IGeneProps,
    { geneUrl: string }
> {
    geneSelectList: any;
    ...
    buildGeneQueryUrl(gene: Gene): string {
        var baseUrl =
            'http://www.fudan-pgx.org/premedkb/index.html#/search/result';
        var label = '?view=widget';
        var queryStr =
            '&queryType=3&num=1&step=1&term=%27' +
            gene.hugoGeneSymbol +
            '%27%5Bgene%5D';
        ...
        return (
            <div>
                <If condition={this.props.genes.length >= 1}>
                    <select
                        ref={el => (this.geneSelectList = el)}
                        style={{ marginBottom: 5 }}
                        onChange={this.handleSelection}
                    >
                        {_.map(this.props.genes, (gene: Gene) => (
                            <option value={gene.hugoGeneSymbol}>
                                {gene.hugoGeneSymbol}
                            </option>
                        ))}
                    </select>

...
export async function fetchPathwaysWithGeneSymbols(
    hugoGeneSymbols: string[],
    species?: string
) {
    function convertData(
        pathwayRawRecord: PathwayRawRecord,
        gene: string
    ): PathwayRecord {
        let pathway = {
    ...
    const pathwayRecords: PathwayRecords = await new Promise<PathwayRecords>(
        (resolve, reject) => {
            // TODO better to separate this call to a configurable client
            request
                .get('https://webservice.wikipathways.org/findPathwaysByText')
                .query({ format: 'json' })
                ...
                .end((err, res) => {
                    if (!err && res.ok) {
                        ...
                        for (let item of filteredResult) {
                            ...
                            ret[item.id] = convertData(
                                item,
                                hugoGeneSymbols.join(',')
                            );
                        }
                        resolve(ret);
                    } else {
